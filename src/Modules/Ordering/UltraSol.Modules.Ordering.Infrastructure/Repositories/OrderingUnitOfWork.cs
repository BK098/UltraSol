...
public sealed class OrderingUnitOfWork(OrderingDbContext context, IDomainEventDispatcher dispatcher)
    : UnitOfWork(context, dispatcher), IOrderingUnitOfWork
{
    ...
    public new async Task<ITransaction> BeginTransactionAsync(CancellationToken cancellationToken = default) =>
        new OrderingTransaction(await Translate(() => base.BeginTransactionAsync(cancellationToken)));
    ...
    private static async Task Translate(Func<Task> action)
    {
        ...
        {
            throw new OrderingFailure(409, "OrderingConflict", "Ordering data changed or conflicts with an existing record.");
        }
    }
    ...
    private sealed class OrderingTransaction(ITransaction transaction) : ITransaction
    {
        public Task CommitAsync(CancellationToken cancellationToken = default) => Translate(async () =>
        {

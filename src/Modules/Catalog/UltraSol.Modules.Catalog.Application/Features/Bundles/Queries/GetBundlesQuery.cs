...
using static UltraSol.Modules.Catalog.Application.Features.Bundles.Queries.GetBundlesQuery;

namespace UltraSol.Modules.Catalog.Application.Features.Bundles.Queries;

public sealed record GetBundlesQuery(PagedFilter? Filter, ProductItemStatus? Status = null,
    Guid? ProductId = null) : IQuery<ApiResult<PaginatedResult<GetBundlesQuery.Response>>>
{
...
public sealed class GetBundlesValidator : AbstractValidator<GetBundlesQuery>
{
    public GetBundlesValidator()
    {
...
internal sealed class GetBundlesQueryHandler(ICatalogReadStore reads) : IQueryHandler<GetBundlesQuery, ApiResult<PaginatedResult<GetBundlesQuery.Response>>>
{
    public async Task<ApiResult<PaginatedResult<GetBundlesQuery.Response>>> Handle(GetBundlesQuery request, CancellationToken ct)
    {
        var page = CatalogQueryPaging.Create(request.Filter!, "Sku");
        var data = await reads.ItemsAsync(page, new(request.ProductId, request.Status, true), ct);

...
using static UltraSol.Modules.Catalog.Application.Features.ProductItems.Queries.GetProductItemDetailQuery;

namespace UltraSol.Modules.Catalog.Application.Features.ProductItems.Queries;

public sealed record GetProductItemDetailQuery(Guid ProductId, Guid ProductItemId) : IQuery<ApiResult<GetProductItemDetailQuery.Response>>
{
...
public sealed class GetProductItemDetailValidator : AbstractValidator<GetProductItemDetailQuery>
{
    public GetProductItemDetailValidator()
    {
...
internal sealed class GetProductItemDetailQueryHandler(ICatalogReadStore reads) : IQueryHandler<GetProductItemDetailQuery, ApiResult<GetProductItemDetailQuery.Response>>
{
    public async Task<ApiResult<GetProductItemDetailQuery.Response>> Handle(GetProductItemDetailQuery request, CancellationToken ct)
    {
        var data = await reads.ItemAsync(request.ProductItemId, request.ProductId, false, ct);
        var row = data;

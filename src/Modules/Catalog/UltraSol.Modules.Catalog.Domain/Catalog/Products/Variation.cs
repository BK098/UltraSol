...
public sealed class Variation : BaseEntity
{
    ...
    private Variation() { Name = null!; }
    private readonly List<VariationOption> _options = [];
    internal void LoadOptions(IEnumerable<VariationOption> options)
    {
        _options.Clear(); _options.AddRange(options);
    }
    public string Name { get; private set; }
    public IReadOnlyList<VariationOption> Options => _options.AsReadOnly();
    internal Variation(string name)
    {
        Name = Guard.Required(name, "Variation name");
    }
    ...
    internal void RemoveOption(Guid id)
    {
        _options.Remove(Find(id));
    }

...
namespace UltraSol.Modules.Catalog.Domain.Catalog.ProductItemAggregate;
...
public sealed class ProductItem : AggregateRoot
{
    ...
    public SKU Sku { get; }
    public IReadOnlyList<OptionSelection> OptionSelections { get; }
    public OptionSignature OptionSignature { get; }
    public BundleDefinition? BundleDefinition { get; }
    public bool IsBundle => BundleDefinition is not null;
    public IReadOnlyList<ProductItemMedia> Media => _media.AsReadOnly();

    private ProductItem(Guid id, Guid productId, SKU sku, OptionSelection[] selections, BundleDefinition? bundle)
        : base(id)
    {
    ...
    public static ProductItem Create(Product product, SKU sku, IEnumerable<OptionSelection> selections) =>
        CreateCore(product, sku, selections, null);
    ...
    {
        ArgumentNullException.ThrowIfNull(product);
        if (product.Id != ProductId) throw new DomainException("Product does not own this item.");
    ...
    public void SetPrimaryMedia(Product product, Guid mediaId)
    {
        ...
        foreach (var media in _media) media.SetPrimary(media == selected);
    }
